...
for j in range(number_of_classes):
    class_dir = os.path.join(DATA_DIR, str(j))
    print(f'Collecting sign language data for {alphabet_digits[j]} ({j})')
    print('Press "R" to start capturing data for this class.')
    ...
    while True:
        ...
        if key == ord('q') or cv2.getWindowProperty('frame', cv2.WND_PROP_VISIBLE) < 1:
            print("Exiting sign language data collection...")
            cap.release()
    ...
    while counter < dataset_size:
        ...
        if cv2.getWindowProperty('frame', cv2.WND_PROP_VISIBLE) < 1:
            print("Window closed by user. Exiting sign language data collection...")
            break
